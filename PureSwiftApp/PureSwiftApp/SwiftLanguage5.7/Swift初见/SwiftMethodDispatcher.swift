...
//        greetings(person: MisunderstoodPerson())
        ...
        // 5-2.code
        // 一：如果用协议引用类型调用
        //   1.如果只定义在 extension 中的方法，直接静态派发
        //   2.如果定义在 protocol 中，实现协议的基类中必须有方法的实现，否则还是会调用协议扩展中的实现（从协议的基类查询有无实现，没有的话直接使用 protcol extesnion 中的实现，有则查找具体类的实现）
        // 二：如果使用类引用(跟协议中有没有声名没关系)
        //   1.若该类继承的基类链条中都没有方法的实现，则会调用协议扩展中的实现
        //   2.若该类继承的基类链条中已有该方法的实现，则会调用该类的实现，函数表派发实现多态
